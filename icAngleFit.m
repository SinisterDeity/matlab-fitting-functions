...
for i = 1:length(obj)
    ...
    opts.Display = 'Off';
    opts.Lower = [0 1 0 1];
    opts.Upper = [Inf 5 Inf 15];
    opts.MaxFunEvals = 1000;
    opts.MaxIter = 500;
    opts.Robust = 'Bisquare';
    opts.StartPoint = [1 3 1 12];
    opts.TolFun = 1e-9;
    opts.TolX = 1e-9;
    ...
    if(bool2Plot)
        ...
        if(isscalar(obj))
            temp = [];
            temp.i0 = i0sGammas(i,1);
            temp.gamma = i0sGammas(i,2);
            ics = singleLorentz(temp,angles);
            plot(angles,ics,'k','LineWidth',2,'DisplayName','Lorentzian 1');
            temp.i0 = i0sGammas(i,3);
            temp.gamma = i0sGammas(i,4);
            ics = singleLorentz(temp,angles);
